...
/*
   ...
   Modified 2026-09-16 by Mobileware Studio for Photo Transfer:
   authenticated connections reject a guest or null session fallback before
   TREE_CONNECT; DFS trees are rejected and share-required encryption is
   honoured; file and directory opens deny write/delete sharing and open final
   reparse points themselves.
   The NativeSMB README lists every change to libsmb2.
                    ...
                    int caller_frees_pdu)
{
        ...
        req.share_access = SMB2_FILE_SHARE_READ; /* Photo Transfer: guarded handle. */
        req.create_disposition = SMB2_FILE_OPEN;
        req.create_options = SMB2_FILE_DIRECTORY_FILE | SMB2_FILE_OPEN_REPARSE_POINT;
        req.name = path;
                ...
                void *command_data, void *private_data)
{
        ...
        /* Photo Transfer: do not follow DFS, and honor share-required encryption. */
        struct smb2_tree_connect_reply *rep = command_data;
        if ((rep->share_flags & (SMB2_SHAREFLAG_DFS | SMB2_SHAREFLAG_DFS_ROOT)) ||
            (rep->capabilities & SMB2_SHARE_CAP_DFS)) {
                smb2_close_context(smb2);
                c_data->cb(smb2, -ENOTSUP, NULL, c_data->cb_data);
                free_c_data(smb2, c_data);
                return;
        }
        if (rep->share_flags & SMB2_SHAREFLAG_ENCRYPT_DATA) {
                if (smb2->dialect < SMB2_VERSION_0300 || !smb2->session_key) {
                        smb2_close_context(smb2);
                 ...
                 void *command_data, void *private_data)
{
        ...
        /* Photo Transfer: required signing also marks non-guest authentication.
         * Reject a server's guest/null fallback before sending TREE_CONNECT. */
        if ((smb2->security_mode & SMB2_NEGOTIATE_SIGNING_REQUIRED) &&
            (rep->session_flags & (SMB2_SESSION_FLAG_IS_GUEST | SMB2_SESSION_FLAG_IS_NULL))) {
                smb2_close_context(smb2);
                c_data->cb(smb2, -EACCES, NULL, c_data->cb_data);
                free_c_data(smb2, c_data);
                return;
...
{
        SMB2_LIST_REMOVE(&smb2->pta_open_handles, fh);
        free(fh);
}

void
smb2_free_open_handles(struct smb2_context *smb2)
{
        while (smb2->pta_open_handles) {
                free_smb2fh(smb2, smb2->pta_open_handles);
        }
}
                ...
                int caller_frees_pdu)
{
        ...
        SMB2_LIST_ADD(&smb2->pta_open_handles, fh);
        fh->cb = cb;
        fh->cb_data = cb_data;
        ...
        req.create_options = create_options | SMB2_FILE_OPEN_REPARSE_POINT;
        req.name = path;
                 ...
                 smb2_command_cb cb, void *cb_data)
{
        ...
        cr_req.file_attributes = SMB2_FILE_ATTRIBUTE_DIRECTORY;
        cr_req.share_access = SMB2_FILE_SHARE_READ; /* Photo Transfer: guarded handle. */
        cr_req.create_disposition = SMB2_FILE_CREATE;
        cr_req.create_options = SMB2_FILE_DIRECTORY_FILE | SMB2_FILE_OPEN_REPARSE_POINT;
        cr_req.name = path;
