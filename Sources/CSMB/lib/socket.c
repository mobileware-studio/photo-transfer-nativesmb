...
/*
   ...
   a client delivers application replies only when they meet the negotiated
   signing and encryption policy.
...
read_more_data:
        ...
        /* Photo Transfer: enforce the negotiated receive policy before delivering
         * application replies. Negotiation/session setup establish the keys;
         * successfully decrypted messages already carry integrity protection. */
        if (!smb2_is_server(smb2) && !has_xfrmhdr &&
            smb2->hdr.command != SMB2_NEGOTIATE &&
            smb2->hdr.command != SMB2_SESSION_SETUP) {
                if (smb2->seal) {
                        smb2_set_error(smb2, "Encryption required for received PDU");
                        return -1;
                }
                if (smb2->sign && smb2->hdr.status != SMB2_STATUS_PENDING &&
                    !(smb2->hdr.flags & SMB2_FLAGS_SIGNED)) {
                        smb2_set_error(smb2, "Signing required for received PDU");
                        return -1;
                }
        }
