...
/*
   ...
   the active-context list is locked for insertion, removal and membership
   checks; local file handles still open when a context is destroyed are freed
   after its outstanding callbacks drain.
 ...
 */
#include <pthread.h>
static pthread_mutex_t active_contexts_lock = PTHREAD_MUTEX_INITIALIZER;
...
void smb2_destroy_context(struct smb2_context *smb2)
{
        ...
        /* Pending callbacks have drained; release handles that never reached CLOSE. */
        smb2_free_open_handles(smb2);
        pthread_mutex_lock(&active_contexts_lock);
